...
def main(lesion):
    ...
    for idx, path in enumerate(tqdm(paths, desc="Processing images")):
        ...
        if img_gt is not None:
            ...
            wandb.log({
                "PSNR": float(f"{psnr:.2f}"),
                "SSIM": float(f"{ssim:.4f}"),
                "PSNR_Y": float(f"{psnr_y:.2f}"),
                "SSIM_Y": float(f"{ssim_y:.4f}"),
                "PSNRB": float(f"{psnrb:.2f}"),
                "PSNRB_Y": float(f"{psnrb_y:.2f}")
            })
